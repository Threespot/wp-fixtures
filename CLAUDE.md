...
`threespot/wp-fixtures` is a Composer **dev-dependency** that ships a WP-CLI command (`wp threespot fixtures …`) for loading example pages and Gravity Forms into a fresh WordPress site. It is part of the Threespot Base Build framework — the architecture spec lives outside this repo at `~/Desktop/threespot-base-build-architecture.md` (sections "Example Content Fixtures", lines 488–646). Decisions like "dev-dependency only", "local-only loading", and "no per-site fixture overrides" are deliberate; don't add knobs the spec excludes.

Note: this package intentionally diverges from the spec on one point — **taxonomy term fixtures were removed**. Only pages and Gravity Forms are loaded. Don't re-add a `TaxonomyLoader` or `fixtures/taxonomies/` to "match the spec."
...
- **Load order is load-bearing.** `FixturesCommand::load()` always runs forms → pages because page block markup may reference imported Gravity Forms (by ID). Don't reorder, and if you add a new fixture type, slot it into the dependency chain consciously.
- **Two different idempotency strategies for two fixture types.** Pages use a `postmeta` marker (`_threespot_fixture_source`). Forms use a single site `option` (`_threespot_fixtures_loaded`) holding an array of imported paths — because GF forms aren't WP posts, there's no postmeta to hang off. `FixturesCommand::status()` queries both directly; don't break that contract without updating it.
- **`PageLoader` defaults and `PageExporter` omissions are paired.** The exporter writes front-matter only for fields whose values differ from the loader's defaults (`status: publish`, `post_type: page`, `menu_order: 0`, `author: admin`, slug derived via `sanitize_title()`, no template override). If you change a default on one side, change it on the other — otherwise roundtripping a fixture mutates it. The `author` default lives in `PageLoader::DEFAULT_AUTHOR_LOGIN`, which the exporter also reads — keep them sharing that constant rather than hardcoding `'admin'` twice.
...
```bash
wp threespot fixtures load --forms               # one section at a time
